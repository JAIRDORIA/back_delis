...
def listado_proveedores():
    try:
        ...
        lista = []
        for fila in datos:
            pro = proveedor(fila[0], fila[1], fila[2], fila[3], fila[4], fila[5])
...
def registro_proveedor(id, nombre, telefono, direccion, email):
    try:
        ...
        cursor.close()
        return proveedor(nuevo_id, nombre, telefono, direccion, email).todic(), None
    except Exception as e:
        raise Exception(str(e))
...
def actualizar_proveedor(id, nombre, telefono, direccion, email, activo):
    try:
        ...
        con.commit()
        cursor.close()
        return proveedor(id, nombre, telefono, direccion, email, activo).todic(), None
    except Exception as e:
        raise Exception(str(e))
